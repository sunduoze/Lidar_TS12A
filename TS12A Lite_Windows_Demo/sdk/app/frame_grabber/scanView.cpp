...
extern uint16_t sample_point_per_circle;
extern uint16_t sample_cnt_per_circle;
extern uint16_t sample_rate;
extern uint16_t sample_accuracy;

const int DEF_MARGIN = 20;
const int DISP_RING_ABS_DIST = 100;
const float DISP_FULL_DIST   = 10000;
const float DISP_MIN_DIST    = 100;//enzo modify 1000
...
void CScanView::onDrawSelf(CDCHandle dc)
{
    ...
    if ((int)_scan_data.size() > picked_point) {
		...
		sprintf(txtBuffer, "Current  speed:    %.2fr/s",_scan_data[picked_point].speed);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 20, txtBuffer);

		sprintf(txtBuffer, "Angle offset:    %.2f degree", _scan_data[picked_point].angleoffset);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 60, txtBuffer);

		sprintf(txtBuffer, "Current  angle:    %.2f degree", _scan_data[picked_point].angle);
        memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 100, txtBuffer);

		sprintf(txtBuffer, "Current  signal:    %d", _scan_data[picked_point].quality);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 140, txtBuffer);

		sprintf(txtBuffer, "Current distance: %.dmm", (_scan_data[picked_point].dist));
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 180, txtBuffer);

		sprintf(txtBuffer, "Sample cnt per circle: %d", sample_cnt_per_circle);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 220, txtBuffer);

		sprintf(txtBuffer, "Sample points per circle: %d", sample_point_per_circle);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 260, txtBuffer);

		sprintf(txtBuffer, "Angle accuracy: %.3f degree", 360.0 / sample_point_per_circle);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 300, txtBuffer);

		sample_rate = uint16_t(sample_point_per_circle * _scan_data[picked_point].speed);

		sprintf(txtBuffer, "Sample rate: %.3f Ksps", sample_rate / 1000.0f);
		memDC.TextOutA(DEF_MARGIN, DEF_MARGIN + 340, txtBuffer);
...
void CScanView::setScanData(RSLIDAR_SIGNAL_DISTANCE_UNIT_T *buffer, size_t count)
{
    _scan_data.clear();
	//_cprintf("setScanData : %d\n", count);
	for (int pos = 0; pos < (int)count; ++pos) 
	{
		scanDot dot;
		if (!buffer[pos].distanceValue) continue;
		dot.speed = buffer[pos].speed;
		dot.angleoffset = buffer[pos].angleoffset;
		dot.quality = buffer[pos].signalValue;
		dot.angle	= ((float)(buffer[pos].angle))/100.0f;
		dot.dist	= buffer[pos].distanceValue/4.0f;      //modify by enzo buffer[pos].distanceValue/4.0f;
		//协议中分辨率：实际测量数据 = 通讯中数值 * 分辨率
		//实际转速 = 通讯中转速数值 * 分辨率(0.05r / s)
		//实际距离 = 通讯中距离数值 * 分辨率(0.25mm)
		//实际角度 = 通讯中角度数值 * 分辨率(0.01°)
