...
//#include <ioctl.h>
uint16_t sample_point_per_circle;
uint16_t sample_cnt_per_circle;
uint16_t sample_rate;
uint16_t sample_accuracy;
...
u_result RSlidarDriverSerialImpl::_cacheScanData()
{
	...
	while (_isScanning)
	{
		...
		_lock.unlock();
		count = NUMBER_OF_TEETH;
		//_cprintf("_cached_scan_node_count : %d  count : %d\n", _cached_scan_node_count, count);
	}
	_isScanning = false;
	return RESULT_OK;
}
...
u_result RSlidarDriverSerialImpl::_waitScanData(RSLIDAR_SIGNAL_DISTANCE_UNIT_T * nodebuffer, size_t & count, _u32 timeout)
{
	...
	//while (waitTime <= timeout && recvNodeCount < count)
	{
		...
		if (crcCheckNum == calcchecksum)
		{
			...
			if (flagOfNewNodes)
			{
				...
				for (int i = 0; i < sampleNumber; i++)
				{
					...
					(nodebuffer + sampleNumberCount)->signalValue = recvBuffer[15 + i * 3];//enzo modify recvBuffer[13 + i * 3]
					(nodebuffer + sampleNumberCount)->angleoffset = angleoffset;
					(nodebuffer + sampleNumberCount)->distanceValue = (recvBuffer[16 + i * 3] << 8) | recvBuffer[17 + i * 3];// enzo modify(recvBuffer[14 + i * 3] << 8) | recvBuffer[15 + i * 3]
				...
				recvNodeCount++;
				//_cprintf("sampleNumberCount : %d, start_angle :%d end_angle : %d\n", sampleNumberCount, angleRange, angleRange + (sampleNumber -1) * angleStep);
			}			
		}
		else
		{
		...
		{
			_cprintf("sampleNumberCount : %d   count : %d\n", sampleNumberCount, count);
			sample_point_per_circle = sampleNumberCount;
			sample_cnt_per_circle = count;
		...
		case rs::hal::Event::EVENT_OK:
			{
				...
				//_cprintf("count : %d   _cached_scan_node_count : %d\n", count, size_to_copy);
...
uint16_t RSlidarDriverSerialImpl::_crc16(uint8_t *startByte, uint16_t numBytes)
{
	...
	uint16_t uIndex;                           // CRC���ұ���ָ��
	while (numBytes--)
	{
		uIndex = uchCRCLo ^ *startByte++;      // ����CRC
